    ...
    haveCameraInfo(false)
{
  ...
  string training;
  ros::param::get("~training", training);// Should be a boolean
  if(training == "on"){
      trainingToggle = true;
  }
  ...
  dynamic_reconfigure::Server<cube_dot_finder::DotFinderConfig>::CallbackType dynamicReconfigCallback;
  ...
  ROS_INFO("[cube_dot_finder]Subscribing to %s/image_raw", this->topic.c_str());
...
void DotFinder::createPublishers(){
    ...
    this->pubImage = image_transport.advertise(topic + "/image_with_detections", 1);
    this->pubDotHypothesis = this->nodeHandle.advertise<dot_finder::DuoDot>(topic  + "/dots", 1);
}

void DotFinder::createSubscribers(){
   this->subImage = this->nodeHandle.subscribe(this->topic + "/image_raw", 1, &DotFinder::imageCallback, this);
   this->subCameraInfo = this->nodeHandle.subscribe(this->topic + "/camera_info", 1, &DotFinder::cameraInfoCallback, this);
}
...
void DotFinder::cameraInfoCallback(const sensor_msgs::CameraInfo::ConstPtr& pMsg){
  if (!this->haveCameraInfo)
  {
    ...
    //--Correct camera_info of tryphon241:--------
    cameraDistortionCoeffs.clear();
    cameraDistortionCoeffs.push_back(-0.935346); cameraDistortionCoeffs.push_back(1.214711); cameraDistortionCoeffs.push_back(0.046259); cameraDistortionCoeffs.push_back(-0.006668); cameraDistortionCoeffs.push_back(0);
    this->cameraMatrixK.at<double>(0, 0) = 1058.448236;
    this->cameraMatrixK.at<double>(0, 1) = 0;
    this->cameraMatrixK.at<double>(0, 2) = 313.849999;
    this->cameraMatrixK.at<double>(1, 0) = 0;
    this->cameraMatrixK.at<double>(1, 1) = 1046.664871;
    this->cameraMatrixK.at<double>(1, 2) = 235.554288;
    this->cameraMatrixK.at<double>(2, 0) = 0;
    this->cameraMatrixK.at<double>(2, 1) = 0;
    this->cameraMatrixK.at<double>(2, 2) = 1;

    cameraMatrixP.at<double>(0, 0) = 966.928955;
    cameraMatrixP.at<double>(0, 1) = 0;
    cameraMatrixP.at<double>(0, 2) = 309.446133;
    cameraMatrixP.at<double>(0, 3) = 0;
    cameraMatrixP.at<double>(1, 0) = 0;
    cameraMatrixP.at<double>(1, 1) = 990.194580;
    cameraMatrixP.at<double>(1, 2) = 243.980756;
...
void DotFinder::imageCallback(const sensor_msgs::Image::ConstPtr& pImageMsg)
{
  ...
  try{
    cv_ptr = cv_bridge::toCvCopy(pImageMsg, sensor_msgs::image_encodings::MONO8);// MONO
  }
...
void DotFinder::detectAndPublishMarkers(cv::Mat &pImage){
  ...
  this->markerDetector.dotFilteringCube(pImage,
                                           this->pairDistorted,
 ...
 */
void DotFinder::dynamicParametersCallback(cube_dot_finder::DotFinderConfig &pConfig, uint32_t pLevel){
  ...
  if(pConfig.toggleROI){
     ...
     regionOfInterest = cv::Rect(0, 0, 640, 480);
  }

  // Detector Parameters
  this->markerDetector.setDetectorParameter(pConfig.threshold,
                                            pConfig.dilation_size,
