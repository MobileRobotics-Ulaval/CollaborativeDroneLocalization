...
ARPE::ARPE(ros::NodeHandle node): m_nodehandle(node), m_joyInitiated(false), m_navInitiated(false){
    ...
    m_pub_twist = node.advertise<geometry_msgs::Twist>("/cmd_vel", 1);
    m_pub_v3 = node.advertise<geometry_msgs::Vector3>("/joy_vel", 1);

    m_joy_sub = node.subscribe("/joy", 1, &ARPE::joyCallback, this);
    m_nav_sub = node.subscribe("/ardrone/navdata", 1, &ARPE::navCallback, this);
    m_pub_empty_reset = node.advertise<std_msgs::Empty>("/ardrone/reset", 1);
    m_pub_empty_takeoff = node.advertise<std_msgs::Empty>("/ardrone/takeoff", 1);
    m_pub_empty_land = node.advertise<std_msgs::Empty>("/ardrone/land", 1);
}

void ARPE::joyCallback(const sensor_msgs::Joy& joy_msg_in){
    if(joy_msg_in.buttons[4]){
...
void ARPE::loop(){
    ...
    while(ros::ok()){
        ...
        if (m_buttonB){
            while (m_drone.state == 3 || m_drone.state == 4){
...
int main(int argc, char* argv[]){
    ...
    ros::init(argc, argv,  "ardrone_pose_estimator_node");
